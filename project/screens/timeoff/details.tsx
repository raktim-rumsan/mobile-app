...
import { useTimeOffById } from '@/project/queries/timeoff-req.query';
...
const TimeoffDetails = () => {
  ...
  const { type, status, description: reason, daysDetails } = data;
  ...
  const formattedDates = Object.entries(daysDetails || {}).map(
    ([date, detail]) => ({
      date: format(new Date(date), 'dd MMMM, yyyy'),
      type:
        (detail as any).timeOffDuration
          ?.replace('_', ' ')
